<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (empty($_POST['uName'])) {
			$uNameErr = "^ Please enter your First Name / Username";
			$contnameErr = "Please enter your Name";
			array_push($errors, $errors);
			array_push($conterrors, $conterrors);
		} else {
			$uName = testInput($_POST["uName"]);
			if (!preg_match("/^[a-zA-Z ]*$/",$uName)) {
				$uNameErr = "^ Only letters and white space allowed";
				$contnameErr = "Only letters and white space allowed";
				array_push($errors, $errors);
				array_push($conterrors, $conterrors);
			}
		}
		...
		if (empty($_POST['email'])) {
			...
			$contemailErr = "Please enter your Email";
			array_push($errors, $errors);
			array_push($conterrors, $conterrors);
		} else {
			...
			if (!filter_var($email,FILTER_VALIDATE_EMAIL)) {
				...
				$contemailErr = "Invalid Email format";
				array_push($errors, $errors);
				array_push($conterrors, $conterrors);
			}
			$emailenter = $_POST['email'];
			$emailcheck = "SELECT * FROM `users` WHERE Email = '".$emailenter."'";
		...
		if (!empty($_POST['contactnum'])) {
			...
			if (!preg_match("/^[[09]|[|\d{2}|\d{3}]][-|\s][\d{9}|\d{7}]*$/",$contactnum)) {
				$contactnumErr = "^ Invalid Phone Number format";
				$contcontactnumErr = "Invalid Phone Number format";
				array_push($errors, $errors);
				array_push($conterrors, $conterrors);
			}
		}
		...
		if (isset($psw) && $passlength >= 8) {
			$psw = $_POST['psw'];
			$rePass = $_POST["rePass"];
			if (empty($_POST['rePass'])) {
				$repswErr = "^ Please confirm your password";
				array_push($errors, $errors);
			} elseif ($psw != $rePass) {
				$repswErr = "^ Passwords Don't Match";
				array_push($errors, $errors);
			}
		}
		...
		if (count($errors) == 0) {
				$psw = sha1($psw);
				$sql = "INSERT INTO users (Username, LastName, Email, ContactNumber, Password) VALUES (?,?,?,?,?)";
		...
		if (isset($_POST['login_submit'])) {
			if (empty($_POST['loginName']) || empty($_POST['loginPass'])) {
				$inputErr = "Please enter your Username or Password";
			} else {
				$loginName = $_POST['loginName'];
				$loginPass = sha1($_POST['loginPass']);
				$loginsqli = "SELECT * FROM `users` WHERE (Username = '".$loginName."' or Email = '".$loginName."') AND Password = '".$loginPass."'";
				$result = mysqli_query($mysqli, $loginsqli);
				$count = mysqli_num_rows($result);

				

				if ($count == 1) {
					$query = mysqli_query($mysqli, "SELECT * FROM `users` WHERE (Username = '".$loginName."' or Email = '".$loginName."') AND Password = '".$loginPass."'");
		...
		# For Feedback Forms...
		// Validation for Name, Email, ContactNumber done above...
		if (empty($_POST["comment"])) {
			$contcommentErr = "Please enter your comment";
			array_push($conterrors, $conterrors);
		} else {
			$comment = testInput($_POST["comment"]);
		}

		if (count($conterrors) == 0)  {
			...
			$sql = "INSERT INTO contacts (Name, Email, ContactNumber, Comment) VALUES (?,?,?,?)";
			...
			$stmt->execute();
			mysqli_query($mysqli, $sql);
			//$alert = "Thank you for contacting us! We appreciate it.";
			//exit();
		}
	...
	//changing account details in myaccount.php
	if (isset($_POST['newchanges'])) {
		$userinfo_sql = mysqli_query($mysqli, "SELECT * FROM users WHERE id = '". $_SESSION['id'] ."' AND Username = '". $_SESSION['username'] ."'");

		if ($_POST['myfirstname'] != $_SESSION['username']) {
			if (empty($_POST['myfirstname'])) {
				$myfirstnameNotice =  "Please enter your First Name";
			} else {
				$myfirstname = testInput($_POST["myfirstname"]);
				if (!preg_match("/^[a-zA-Z ]*$/",$myfirstname)) {
					$myfirstnameNotice = "Only letters and white space are allowed";
				} else {
					$firstname_update_sql = "UPDATE users SET Username = '". $myfirstname ."' WHERE id = '". $_SESSION['id'] ."'";
					$myfirstnameSuccess = "Successfully changed";
					$firstname_update_sqlresult = mysqli_query($mysqli, $firstname_update_sql);
					$_SESSION['username'] = $myfirstname;
				}
			}
		}
		if ($_POST['mylastname'] != $_SESSION['lastname']) {
			if (empty($_POST['mylastname'])) {
				$mylastnameNotice =  "Please enter your Last Name";
			} else {
				$mylastname = testInput($_POST["mylastname"]);
				if (!preg_match("/^[a-zA-Z ]*$/",$mylastname)) {
					$mylastnameNotice = "Only letters and white space are allowed";
				} else {
					$lastname_update_sql = "UPDATE users SET LastName = '". $mylastname ."' WHERE id = '". $_SESSION['id'] ."'";
					$mylastnameSuccess = "Successfully changed";
					$lastname_update_sqlresult = mysqli_query($mysqli, $lastname_update_sql);
					$_SESSION['lastname'] = $mylastname;
				}
			}
		}
		if ($_POST['myemail'] != $_SESSION['email'] ) {
			if (empty($_POST['myemail'])) {
				$myemailNotice = "Please enter your Email";
			} else {
				$myemail = testInput($_POST["myemail"]);
				if (!filter_var($myemail,FILTER_VALIDATE_EMAIL)) {
					$myemailNotice = "Invalid Email format";
				}
				$myemailenter = $_POST['myemail'];
				$myemailcheck = "SELECT * FROM `users` WHERE Email = '".$myemailenter."'";
				$myemailresult = mysqli_query($mysqli, $myemailcheck);
				$mycount = mysqli_num_rows($myemailresult);

				if ($mycount == 1) {
					$myemailNotice = "Email already taken";
				} else {
						$email_update_sql = "UPDATE users SET Email = '". $myemail ."' WHERE id = '". $_SESSION['id'] ."'";
						$myemailSuccess = "Successfully changed";
						$email_update_sqlresult = mysqli_query($mysqli, $email_update_sql);
						$_SESSION['email'] = $myemail;
				}
			}
		}
		if ($_POST['mynumber'] != $_SESSION['contactnumber']) {
			if (empty($_POST['mynumber'])) {
				$mynumberNotice = "Please enter Your Phone Number";
			} else {
				$mynumber = testInput($_POST['mynumber']);
				if (!preg_match("/^[[09]|[|\d{2}|\d{3}]][-|\s][\d{9}|\d{7}]*$/",$mynumber)) {
					$mynumberNotice = "Invalid Phone Number format";
				} else {
						$number_update_sql = "UPDATE users SET ContactNumber = '". $mynumber ."' WHERE id = '". $_SESSION['id'] ."'";
						$mynumberSuccess = "Successfully changed";
		...
		if (isset($_SESSION['address'])) {
			if ($_POST['myaddress'] != $_SESSION['address']) {
				if (empty($_POST['myaddress'])) {
					$myaddressNotice = "Please enter Your Address";
				} else {
					$myaddress = testInput($_POST['myaddress']);
					$address_update_sql = "UPDATE users SET Address = '". $myaddress ."' WHERE id = '". $_SESSION['id'] ."'";
						$myaddressSuccess = "Successfully changed";
	...
	} // end if isset new changes

	#Changing Password...
	if (isset($_POST['changepassword'])) {
		if (isset($_POST['oldpassword'])) {
			$oldpass = sha1($_POST['oldpassword']);
				$pass_match_sql = mysqli_query($mysqli,"SELECT Password FROM `users` WHERE Username = '".$_SESSION['username']."' AND id = '". $_SESSION['id'] ."'");
				while ($passmatch = mysqli_fetch_array($pass_match_sql)) {
					$savedpass = $passmatch['Password'];
				}
			if ($oldpass == $savedpass) {
				if (empty($_POST['newpassword'])) {
					...
					$confirmpasslength = strlen($confirmpassword);
					if ($passlength < 8) {
						$newpasswordErr = "^ Password must be at least 8 characters";
					} else {
						if ($newpassword == $confirmpassword) {
							$newpas = sha1($_POST['newpassword']);
							$pass_update_sql = "UPDATE users SET Password = '". $newpas ."' WHERE id = '". $_SESSION['id'] ."' AND Username = '". $_SESSION['username'] ."'";
							$newpasswordSuccess = "Successfully changed";
							$pass_update_sqlresult = mysqli_query($mysqli, $pass_update_sql);
						} else { // if passwords don't match
							$confirmpasswordErr = "Passwords Don't Match";
						}
					}
				}			
			} else { // if the old password doesn't match
				$oldpasswordErr = "That is not your password";
			}
		}
	}
			...
			$del_pass_match_sql = mysqli_query($mysqli,"SELECT Password FROM `users` WHERE Username = '". $_SESSION['username'] ."' AND id = '". $_SESSION['id'] ."'") or die("Failed to Get User's Information");
				while ($pass_match = mysqli_fetch_array($del_pass_match_sql)) {
								...
								if ($mypass != $savedpass) {
									...
									$Sure = 'confirm("Are you sure to continue? This would delete all your information, but recent checked out orders will not be cancelled.")';
									$sorry = 'alert("We\'re sorry to see you go.")';
									echo "<script>";
									echo $Sure;
									echo "</script>";
									if ($Sure == true) {
							...
							} else { // if the user clicks the button, but no password in textbox
								$del_oldpasswordErr = "Please enter you password";
							}
						}
	*/
